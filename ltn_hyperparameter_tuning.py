...
if __name__ == '__main__':
    ...
    def hyper_parameter_tune(config):
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer, config['step'], config['gamma'])
        beta = config['beta']
        for epoch in range(loaded_epoch, num_epochs):
            with ClearCache(device):
                ...
                train.report({"accuracy_recent_coarse": accuracy_recent_coarse[-1],
                              "accuracy_recent_fine": accuracy_recent_fine[-1]})
        ...
        save_evaluation_metric(
            evaluation_metric_train, evaluation_metric_valid, f"{base_path}/result", description)
    ...
    search_space = {
        ...
        "gamma": tune.loguniform(1e-3, 1e-1),  # Problem
        "step": tune.randint(5, 8)
